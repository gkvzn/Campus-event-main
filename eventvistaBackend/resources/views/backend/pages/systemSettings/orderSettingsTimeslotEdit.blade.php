                ...
                <div class="col-xl-9 order-2 order-md-2 order-lg-2 order-xl-1">
                    <form action="{{ route('admin.timeslot.update') }}" method="POST">
                        ...
                        <div class="card mb-4" id="section-1">
                            <div class="card-body">
                                <div class="mb-3">
                                    <label for="timeline" class="form-label">{{ localize('Add Name') }}</label>
                                    <input type="text" id="name" name="name" class="form-control"
                                        placeholder="{{ localize('Add Name') }}" value="{{ $slot->name }}" required>
                                </div>
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    <label for="timeline" class="form-label">{{ localize('Last Order Time') }}</label>
                                    <select name="last_order_time" class="form-control">
                                        @foreach (App\Models\ScheduledDeliveryTimeList::TIME as $key => $item)
                                            <option value="{{ $key }}"
                                                @if ($key == $slot->last_order_time) selected @endif>{{ $item }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                {{-- <div class="mb-3">
                                    <label for="timeline" class="form-label">{{ localize('Stock') }}</label>
                                    <input type="number" id="timeline" name="stock" class="form-control"
                                        placeholder="{{ localize('Stock') }}" value="{{ $slot->stock }}" required>
                                </div> --}}
                                <div class="mb-3">

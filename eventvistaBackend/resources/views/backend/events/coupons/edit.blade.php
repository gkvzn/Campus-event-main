        ...
        <div class="container">
            ...
            <div class="row mb-4 g-4">
                ...
                <div class="col-xl-9 order-2 order-md-2 order-lg-2 order-xl-1">
                    <form action="{{ route('business.events.coupons.update') }}" method="POST" class="pb-650">
                        @csrf
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-4">
                                    @php
                                        ...
                                        if (!is_null(json_decode($coupon->event_category_ids))) {
                                            $coupon_categories = App\Models\Business\EventCategories::whereIn('id', json_decode($coupon->event_category_ids))->pluck('id');
                                        }
                                    @endphp
                                ...
                                <div class="mb-4">
                                    @php
                                        $coupon_events = null;
                                        if (!is_null(json_decode($coupon->event_ids))) {
                                            $coupon_events = App\Models\Business\Events::whereIn('id', json_decode($coupon->event_ids))->pluck('id');
                                        }
                                    @endphp
                                    <label for="parent_id" class="form-label">{{ localize('Events') }}</label>
                                    <select class="form-control select2" class="w-100" data-toggle="select2"
                                        data-placeholder="{{ localize('Select Events') }}" name="event_ids[]" multiple>
                                        @foreach ($events as $event)
                                            <option value="{{ $event->id }}"
                                                @if (!is_null($coupon_events)) {{ $coupon_events->contains($event->id) ? 'selected' : '' }} @endif>
                                                {{ $event->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="number" min="0" step="0.001" class="form-control" id="min_spend"
                                        name="min_spend" required value="{{ $coupon->min_spend }}">
                                </div>
                                <div class="mb-4">

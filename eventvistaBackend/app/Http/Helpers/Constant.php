...
if (!function_exists('newLocalization')) {
    ...
    function newLocalization($lang, $t_key, $key)
    {
        ...
        $localization->t_value = str_replace(["\r", "\n", "\r\n"], '', $key);
        $localization->save();
        // clear cache
...
if (!function_exists('get_next_days')) {
    // file delete
    function get_next_days($days)
    {
        $date = date('Y-m-d');
        $list = [];
        for ($i = 0; $i <= ($days - 1); $i++) {
            ...
            $list[$i]['day'] = $i + 1;
            $list[$i]['name'] = date('D', $addDay);
            $list[$i]['date'] = date('d', $addDay);
        }
...
if (!function_exists('setCoupon')) {
    ...
    function setCoupon($coupon)
    {
        ...
        Cache::put('coupon_code_' . auth('sanctum')->user()->id ?? '', $coupon, 3600);
    }
}
if (!function_exists('removeCoupon')) {
    ...
    function removeCoupon()
    {
        ...
        if (isset(auth('sanctum')->user()->id) && Cache::has('coupon_code_' . auth('sanctum')->user()->id ?? '')) {
            Cache::forget('coupon_code_' . auth('sanctum')->user()->id ?? '');
        }
    }
}
if (!function_exists('getCoupon')) {
    ...
    {
        if (isset(auth('sanctum')->user()->id) && Cache::has('coupon_code_' . auth('sanctum')->user()->id ?? '')) {
            return Cache::get('coupon_code_' . auth('sanctum')->user()->id ?? '');
        }
    ...
    function checkCouponValidityForCheckout($carts)
    {
        if (isset($_COOKIE['coupon_code'])) {
            ...
            if ($coupon) {
                ...
                return [
                    'status' => false,
                    'message' => localize('The coupon is not valid. Please check the code and try again.'),
                ];
            }
        }
...
if (!function_exists('refundPaymentStatus')) {
    // unpaid Payment Status
...
if (!function_exists('OrderConfirmedStatus')) {
    // order cancelled Status
    function OrderConfirmedStatus()
    {
        return 'order-confirmed';
    }
}

if (!function_exists('orderProcessedStatus')) {
    // orderProcessingStatus
    function orderProcessedStatus()
    {
        return 'order-processed';
    }
}

if (!function_exists('orderShippedStatus')) {
    // orderProcessingStatus
    function orderShippedStatus()
    {
        return 'order-shipped';
    }
}

if (!function_exists('OrderInPreparationStatus')) {
    // order cancelled Status
    function OrderInPreparationStatus()
    {
        return 'order-in-preparation';
    }
}

if (!function_exists('OrderOnTheWayStatus')) {
    // order cancelled Status
    function OrderOnTheWayStatus()
    {
        return 'order-on-the-way';
    }
}

if (!function_exists('OrderOnConfirmed')) {
    // order cancelled Status
    function OrderOnConfirmed()
    {
        return 'order-confirmed';
    }
}
...
if (!function_exists('OrderVoidStatus')) {
    // order cancelled Status
    function OrderVoidStatus()
    {
        return 'order-void';
    }
}
...
if (!function_exists('SendNotifications')) {
    function SendNotifications($Payload)
    {
        ...
        $getTokens = Fcmtoken::where('user_id', $Payload['receiver_id'])->get();

        if (count($getTokens) == 0) {
            return false;
        }
        $user = User::where('id', $Payload['receiver_id'])->first();
        if ($user == null || $user->fcm == 0) {
            return false;
        }
        foreach ($getTokens as $key => $item) {
            try {
                $deviceToken = $item->token; // The FCM token of the target device

                $serverKey = env('FIRBASE_SERVER_KEY'); // Your Firebase server key
...
if (!function_exists('LangKey')) {
    function LangKey($lang_key): string
    {
        return $lang_key != null ? $lang_key : env('DEFAULT_LANGUAGE');
    }
}

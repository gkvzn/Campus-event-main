...
class CustomersController extends Controller
{
    ...
    public function repeate_customer(Request $request)
    {

        $searchKey = null;
        $is_banned = null;

        $collect = ModelsOrder::groupBy('user_id')
            ->havingRaw('COUNT(user_id) > 1')->where('payment_status', 'paid')
            ->get();

        $userids = $collect->unique(['user_id']);
        ...
        $customers = User::whereIn('id', $ids)->latest();

        if ($request->search != null) {
            $customers = $customers->where('first_name', 'like', '%'.$request->search.'%')
                ->orWhere('last_name', 'like', '%'.$request->search.'%')
                ->orWhere('email', 'like', '%'.$request->search.'%')->whereIn('id', $ids);
            $searchKey = $request->search;
        }
        ...
        return view('backend.pages.customers.repeate-customer', compact('customers', 'searchKey', 'is_banned'));
